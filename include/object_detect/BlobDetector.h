...
namespace object_detect
{
  ...
  class BlobDetector
  {
    public:
      ...
      std::vector<BallCandidate> detect_lut(cv::Mat in_img, const std::vector<BallTypePtr>& ball_types, cv::OutputArray p_labels_img = cv::noArray());
      std::vector<BallCandidate> detect(cv::Mat in_img, const std::vector<BallTypePtr>& ball_types, cv::OutputArray p_labels_img = cv::noArray());
    ...
    private:
      ...
      cv::Mat binarize_image(cv::Mat hsv_img, cv::Mat lab_img, const SegConfPtr seg_conf) const;
      ...
      cv::Mat threshold_hsv(cv::Mat hsv_img, const SegConfPtr seg_conf) const;
      cv::Mat threshold_lab(cv::Mat lab_img, const SegConfPtr seg_conf) const;
      bool bitwise_and_ocl(uint8_t value, cv::InputArray in_img, cv::OutputArray out_img);

      std::vector<BallCandidate> blobs_to_balls(const std::vector<Blob>& blobs, const BallTypePtr ball_type);
