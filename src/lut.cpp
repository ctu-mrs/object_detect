...
namespace object_detect
{
  ...
  class parallelHSVLUT : public cv::ParallelLoopBody
  {
    public:
        parallelHSVLUT(
            ...
            const double l_lower, const double val_higher,
          ...
          : rgb2hsv_lut(rgb2hsv_lut),
            ...
            val_lower(l_lower), val_higher(val_higher),
  ...
  void add_lut_lab(lut_t& ret, const drcfg_t& drcfg)
  {
  ...
  lut_t generate_lut(const drcfg_t& drcfg)
  {
    ...
    for (size_t it = 0; it < lut_size; it++)
      ret[it] = 0;
    switch (drcfg.binarization_method)
    {
      case bin_method_t::hsv:
        add_lut_hsv(ret, drcfg);
        break;
      case bin_method_t::lab:
        add_lut_lab(ret, drcfg);
        break;
